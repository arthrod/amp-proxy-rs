...
> **致原作者 / Attribution:** 本仓库 fork 自 [margbug01/amp-proxy-rs](https://github.com/margbug01/amp-proxy-rs)，感谢原作者 margbug01 的出色工作。本文档（中文版）基于其原创 README.md，依据 MIT 协议保留归属声明。
...
| 能力 | 说明 |
|---|---|
| 🪶 小体积 release 二进制 | LTO + strip + `opt-level = "z"`，无需额外运行时服务，Linux 版为纯静态二进制（musl） |
...
| 🔐 环境变量替换 | 配置值支持 `${VAR}` 和 `$VAR` 语法，在加载时展开，无需在文件中明文写密钥 |
| 🧪 覆盖验证 | 188 个单元测试，并用真实 Amp CLI 验证 main agent / librarian / finder / DeepSeek tool use |
...
`config.yaml` 中所有字符串值支持在加载时展开 `${VAR}` 和 `$VAR`，未解析的变量替换为空字符串。
...
密钥可通过 shell、systemd `EnvironmentFile`、Docker `--env-file` 或 GitHub Actions secrets 注入，无需写入配置文件。
...
本仓库 fork 自 [margbug01/amp-proxy-rs](https://github.com/margbug01/amp-proxy-rs)。感谢原作者 **margbug01** 的出色设计与实现。
